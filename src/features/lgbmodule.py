...
from src.features.base import BaseGBModule
from .base import MLPFeatureExtractor
...
class LightGBMWithNN:
    def __init__(self, input_dim, mlp_hidden=[128, 64], embed_dim=32, lgb_params=None):
        self.feature_extractor = MLPFeatureExtractor(input_dim, mlp_hidden, embed_dim)
        self.lgb_model = None
    ...
    def fit(self, X, y, group):
        # Train MLP feature extractor (if needed, or assume pretrained)
        X_embed = self.feature_extractor.transform(X)

        train_data = lgb.Dataset(X_embed, label=y, group=group)
        self.lgb_model = lgb.train(self.lgb_params, train_data)

    def predict(self, X):
        X_embed = self.feature_extractor.transform(X)
        return self.lgb_model.predict(X_embed)